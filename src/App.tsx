...
function Contact() {
  ...
  function error(name: keyof FieldErrors) { return errors[name] && <span id={`${name}-error`} className="field-error">{errors[name]}</span> }
  return <section id="contact" className="contact section-space" aria-labelledby="contact-title"><div className="page-width contact-layout"><Reveal className="contact-intro"><p className="eyebrow">LET’S BUILD WHAT’S NEXT</p><h2 id="contact-title">Good technology<br />starts with a<br /><span>conversation.</span></h2><p>Tell us where you are and what you have in mind. We’ll work out the next step together.</p><a className="contact-email" href={`mailto:${site.contactEmail}`}>{site.contactEmail} <Arrow diagonal /></a><div className="contact-brand"><Mark /><span>A stronger foundation.</span></div></Reveal><Reveal><form ref={formRef} onSubmit={submit} noValidate className="contact-form" aria-label="Contact Monolith" aria-busy={status === 'loading'}><div className="form-row"><label>Your name<input name="name" autoComplete="name" maxLength={120} required {...field('name')} />{error('name')}</label><label>Email address<input name="email" type="email" autoComplete="email" maxLength={254} required {...field('email')} />{error('email')}</label></div><div className="form-row"><label>Company<input name="company" autoComplete="organization" maxLength={160} required {...field('company')} />{error('company')}</label><label>Team size <span className="optional">(optional)</span><select name="teamSize" defaultValue=""><option value="">Select team size</option><option value="1-10">1–10 people</option><option value="11-50">11–50 people</option><option value="51-200">51–200 people</option><option value="201+">201+ people</option></select></label></div><label>What can we help with?<select name="service" defaultValue="" required {...field('service')}><option value="" disabled>Select a capability</option>{services.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}<option value="not-sure">Not sure yet</option></select>{error('service')}</label><label>A little about your project<textarea name="message" rows={4} maxLength={5000} placeholder="What’s working, what isn’t, and what would you like to change?" required {...field('message')} />{error('message')}</label><div className="honeypot" aria-hidden="true"><label>Leave this empty<input name="website" tabIndex={-1} autoComplete="off" /></label></div><label className="consent"><input name="consent" type="checkbox" required {...field('consent')} /><span>I agree to Monolith using these details to respond to my inquiry. <a href="#privacy" onClick={openPrivacyDetails}>Privacy information</a></span></label>{error('consent')}<button type="submit" className="button button-light form-submit" disabled={status === 'loading'}>{status === 'loading' ? 'Sending inquiry…' : status === 'error' ? 'Try sending again' : 'Send inquiry'}<Arrow diagonal /></button><div aria-live="polite" aria-atomic="true">{status === 'success' && <p className="form-notice success">Your inquiry has been received. Thank you for getting in touch.</p>}{status === 'error' && <p className="form-notice error">We couldn’t confirm your inquiry was received. Please try again, or <a href={`mailto:${site.contactEmail}`}>contact us by email</a>.</p>}</div></form></Reveal></div></section>
}
...
function Footer() {
  return <footer className="footer page-width"><div className="footer-main"><a className="brand" href="#home" aria-label="Monolith home"><Mark /><span>MONOLITH</span></a><p>IT, built on a stronger foundation.</p><a href="#home" className="back-top">Back to top <span aria-hidden="true">↑</span></a></div><details id="privacy" className="privacy"><summary>Privacy information <span aria-hidden="true">+</span></summary><div><p>When you submit an inquiry, Monolith uses your name, email address, company, service selection, optional team size, and message to review and respond to your request.</p><p>Inquiries are stored privately and automatically deleted after 90 days during daily cleanup. Cloudflare processes and stores this information as our infrastructure provider. This website does not use analytics or advertising tracking.</p><p>To ask about your information or request its deletion, contact <a href={`mailto:${site.contactEmail}`}>{site.contactEmail}</a>. Please do not include passwords or sensitive account details in your inquiry.</p></div></details><div className="footer-bottom"><span>© {new Date().getFullYear()} Monolith</span><span>{site.domain}</span></div></footer>
}

export default function App() {
  useEffect(() => {
    const openPrivacy = () => { if (window.location.hash === '#privacy') { const element = document.getElementById('privacy'); if (element instanceof HTMLDetailsElement) element.open = true } }
    openPrivacy()
