...
// The API rejects invisible control and direction characters (often added when phone numbers are copied)
// and tabs or line breaks in single-line fields, so strip them before validating and sending.
const invisibleCharacters = /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f-\u009f\u202a-\u202e\u2066-\u2069]/g
const lineBreaks = /[\t\n\r\u2028\u2029]+/g
function Contact() {
  const [errors, setErrors] = useState<FieldErrors>({})
  const [status, setStatus] = useState<'idle' | 'loading' | 'success' | 'error' | 'rejected' | 'limited'>('idle')
  const formRef = useRef<HTMLFormElement>(null)
  async function submit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault()
    if (status === 'loading') return
    const data = new FormData(e.currentTarget)
    const value = (name: string, multiline = false) => { const text = String(data.get(name) || '').replace(invisibleCharacters, ''); return (multiline ? text : text.replace(lineBreaks, ' ')).trim() }
    const payload = { name: value('name'), email: value('email'), company: value('company'), service: value('service'), teamSize: value('teamSize'), message: value('message', true), consent: data.get('consent') === 'on', website: value('website') }
    ...
    try {
      ...
      if (result.status === 400) { setStatus('rejected'); return }
      if (result.status === 429) { setStatus('limited'); return }
  ...
  return <section id="contact" className="contact section-space" aria-labelledby="contact-title"><div className="page-width contact-layout"><Reveal className="contact-intro"><p className="eyebrow">LET’S BUILD WHAT’S NEXT</p><h2 id="contact-title">Good technology<br />starts with a<br /><span>conversation.</span></h2><p>Tell us where you are and what you have in mind. We’ll work out the next step together.</p><a className="contact-email" href={`mailto:${site.contactEmail}`}>{site.contactEmail} <Arrow diagonal /></a><div className="contact-brand"><Mark /><span>A stronger foundation.</span></div></Reveal><Reveal><form ref={formRef} onSubmit={submit} noValidate className="contact-form" aria-label="Contact Monolith" aria-busy={status === 'loading'}><div className="form-row"><label>Your name<input name="name" autoComplete="name" maxLength={120} required {...field('name')} />{error('name')}</label><label>Email address<input name="email" type="email" autoComplete="email" maxLength={254} required {...field('email')} />{error('email')}</label></div><div className="form-row"><label>Company<input name="company" autoComplete="organization" maxLength={160} required {...field('company')} />{error('company')}</label><label>Team size <span className="optional">(optional)</span><select name="teamSize" defaultValue=""><option value="">Select team size</option><option value="1-10">1–10 people</option><option value="11-50">11–50 people</option><option value="51-200">51–200 people</option><option value="201+">201+ people</option></select></label></div><label>What can we help with?<select name="service" defaultValue="" required {...field('service')}><option value="" disabled>Select a capability</option>{services.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}<option value="not-sure">Not sure yet</option></select>{error('service')}</label><label>A little about your project<textarea name="message" rows={4} maxLength={5000} placeholder="What’s working, what isn’t, and what would you like to change?" required {...field('message')} />{error('message')}</label><div className="honeypot" aria-hidden="true"><label>Leave this empty<input name="website" tabIndex={-1} autoComplete="off" /></label></div><label className="consent"><input name="consent" type="checkbox" required {...field('consent')} /><span>I agree to Monolith using these details to respond to my inquiry. <a href="/privacy" target="_blank" rel="noopener">Privacy notice<span className="visually-hidden"> (opens in a new tab)</span></a></span></label>{error('consent')}<button type="submit" className="button button-light form-submit" disabled={status === 'loading'}>{status === 'loading' ? 'Sending inquiry…' : status === 'error' || status === 'rejected' ? 'Try sending again' : 'Send inquiry'}<Arrow diagonal /></button><div aria-live="polite" aria-atomic="true">{status === 'success' && <p className="form-notice success">Your inquiry has been received. Thank you for getting in touch.</p>}{status === 'error' && <p className="form-notice error">We couldn’t confirm your inquiry was received. Please try again, or <a href={`mailto:${site.contactEmail}`}>contact us by email</a>.</p>}{status === 'rejected' && <p className="form-notice error">Some of your details couldn’t be accepted. Please check them and try again, or <a href={`mailto:${site.contactEmail}`}>contact us by email</a>.</p>}{status === 'limited' && <p className="form-notice error">We’ve received several inquiries from your connection recently. Please try again in an hour, or <a href={`mailto:${site.contactEmail}`}>contact us by email</a>.</p>}</div></form></Reveal></div></section>
}
